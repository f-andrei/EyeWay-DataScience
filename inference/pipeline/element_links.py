def link_elements(elements, stream_output):
    if stream_output == "none":
        ...
        elements["pgie"].link(elements["nvtracker"])
        elements["nvtracker"].link(elements["nvdsanalytics"])
        elements["nvdsanalytics"].link(elements["nvvidconv_analytics"])
        elements["nvvidconv_analytics"].link(elements["filter_analytics"])
        elements["filter_analytics"].link(elements["queue"])
    ...
    if stream_output in ("file", "rtsp", "display"):
        ...
        elements["nvvidconv_analytics"].link(elements["filter_analytics"])
        elements["filter_analytics"].link(elements["queue"])
        elements["queue"].link(elements["nvtiler"])
